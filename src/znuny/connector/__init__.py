...
class ZnunyConnector:
    ...
    def sql_execute(self, sql, limit=''):
        ...
        print(response.content) if response.status_code != 200 else None
        ...
        for row in rows:
            if 'FilterMessage' not in row.get('class', []):
                cells = row.find_all('td')
                data.append(
                    {headers[i]: cell.get('title') for i, cell in enumerate(cells) if cell.text != 'No matches found.'})

        exit_link = self.protocol + '://' + self.host + '/otrs/index.pl?Action=Logout;ChallengeToken=' + ChallengeToken + ';'
        response = session.get(exit_link)
        print(response.content) if response.status_code != 200 else None
        session.close()
        return data
